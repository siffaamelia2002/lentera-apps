...
// Komponen Skeleton untuk efek loading yang lebih halus
    ...
    <div className="p-6 lg:p-10 space-y-12 animate-pulse">
      ...
      <div className="flex justify-between items-center">
        <div className="space-y-3">
          <div className="h-8 w-64 bg-emerald-500/20 rounded-md"></div>
          <div className="h-4 w-40 bg-emerald-500/10 rounded-md"></div>
        </div>
        <div className="h-12 w-12 bg-emerald-500/20 rounded-full"></div>
      </div>
      ...
      <div className="space-y-12">
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6">
          {[1, 2, 3, 4].map((i) => (
            <div key={i} className="h-32 bg-emerald-500/10 rounded-xl border border-emerald-500/5"></div>
          ))}
        </div>

        {/* Skeleton Chart & Favorite Books */}
        <div className="grid grid-cols-1 lg:grid-cols-3 gap-8">
          <div className="lg:col-span-2 h-96 bg-emerald-500/10 rounded-xl border border-emerald-500/5"></div>
          <div className="h-96 bg-emerald-500/10 rounded-xl border border-emerald-500/5"></div>
        </div>

        {/* Skeleton Recent Activity */}
        <div className="h-72 bg-emerald-500/10 rounded-xl border border-emerald-500/5"></div>
      </div>
    </div>
  );
};
...
export default function AdminDashboardClient() {
  // Fetching menggunakan TanStack Query dan api-client
  const { data, isLoading, isError } = useQuery<DashboardData>({
    queryKey: ["adminDashboardData"],
    queryFn: async () => {
      // Mengikuti path API bawaan dari route Laravel kamu
      // Catatan: Jika baseURL di api-client kamu SUDAH berakhiran '/api',
      // maka cukup tulis '/admindashboard' di sini.
    ...
    return (
      <div className="p-6 lg:p-10">
        <div className="p-4 bg-red-500/10 border border-red-500/20 rounded-lg text-red-500 flex items-center gap-3">
          ...
          <div>
            <p className="font-semibold">Gagal memuat data panel kendali.</p>
